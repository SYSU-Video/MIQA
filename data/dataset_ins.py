...
class MIQAINSDataset(Dataset):
    """
    Dataset for Instance Segmentation-oriented MIQA Task.

    Args:
        det_root (str): Root directory containing images, source (original) images, and additional info.
        ins_root (str): Root directory containing instance segmentation MMOS label CSVs.
        split_file (str, optional): Path to the CSV file specifying train/val split.
        patch_num (int, optional): Number of patches per image (default: 1).
        transforms (Optional[Sequence[Callable]]): Sequence of two transforms:
            [crop_transform, resize_transform].
        ...
        return_all_metrics (bool, optional): Whether to return all metrics (default: False).
    ...
    def __init__(
        ...
        # Set split file if not provided
        ...
        self.coco_annotations = os.path.join(self.det_root, 'additional_info', 'instances_val2017.json')
        ...
        # Initialize distortion level mapping
    ...
    def _initialize_distortion_levels(self):
        """
        Initialize complete level mapping for all distortion types.

        Standard distortion: 0-4 (5 levels)
        bg_stronger: 5-14 (10 levels)
        fg_stronger: 15-24 (10 levels)
        """
        # Standard distortion levels (1-5 mapped to 0-4)
    ...
    def _process_all_samples(self):
        """Process all samples and store their information."""
        # Load COCO annotations
        with open(self.coco_annotations, 'r') as f:
            coco_data = json.load(f)

        # Create image_id -> category mapping
        ...
        for _, row in tqdm(self.split_data.iterrows(), total=len(self.split_data), desc="Processing split"):
            ...
            ori_image_path = os.path.join(self.det_root, 'src_images', folder_name + '.jpg')
            ...
            # Load MMOS CSVs
            mmos_acc_file = os.path.join(self.ins_root, 'labels', f"{folder_name}_accuracy.csv")
            mmos_con_file = os.path.join(self.ins_root, 'labels', f"{folder_name}_consistency.csv")
            ...
            for idx in range(len(mmos_data)):
                ...
                image_path = os.path.join(self.det_root, 'images', folder_name, dist_image_name)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """Return transformed image, label, and metadata."""
    ...
    def get_split_info(self) -> Dict[str, List]:
        """Return split info: image paths, categories, and quality scores."""
        return {
...
if __name__ == '__main__':
    ...
    # Initialize dataset
    dataset = MIQAINSDataset(
        det_root='/public/datasets/miqa_det',
        ins_root='/public/datasets/miqa_ins',
